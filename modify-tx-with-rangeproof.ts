...
const ecdhNoncePreimage = sha256(secp256k1.Point.fromHex(nonceCommitment).multiply(blindingKey).toBytes());

let nonce = BigInt("0x" + Buffer.from(sha256(ecdhNoncePreimage)).toString("hex"));
...
let signatures = Buffer.from(t.outs[index].rangeProof!.subarray(846).toString("hex"), "hex")
...
let valueBigIntArg: bigint = BigInt("0x" + Buffer.from(decryptedByteValues.slice(decryptedByteValues.length - 8)).toString("hex")) //BigInt(0x0000000005f5e0ff);
